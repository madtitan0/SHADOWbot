...
# --- Configuration (V3.2 Strict Fortune) ---
class Config:
    SYMBOLS = ["GC=F"] 
...
class Backtester:
    ...
    def run_sim(self):
        ...
        for i in range(55, len(self.data)):
            ...
            # --- V3.2 "Aggressive Scaler" logic ---
            if self.balance > hwm: hwm = self.balance
            current_dd_pct = (hwm - self.balance) / hwm * 100.0
            
            risk_modifier = 1.0
            if current_dd_pct >= 0.5: risk_modifier = 0.666 # Drop to ~0.50%
            if current_dd_pct >= 1.0: risk_modifier = 0.333 # Drop to ~0.25%
            if current_dd_pct >= 1.5: risk_modifier = 0.133 # Drop to ~0.10%
            ...
            if position == 0 and risk_modifier > 0:
                ...
                if Config.USE_HTF_FILTER:
                    is_extreme_rsi_buy = (rsi < 25) if Config.USE_EXTREME_RSI else False
                    is_extreme_rsi_sell = (rsi > 75) if Config.USE_EXTREME_RSI else False
                    
                    if not is_extreme_rsi_buy:
                        can_buy = close_prev > ema_50_htf
                    if not is_extreme_rsi_sell:
                        can_sell = close_prev < ema_50_htf
                
                # Signal Logic (Same as V3.1)
                # 1. Momentum
                if atr >= min_atr:
    ...
    def generate_report(self):
        ...
        for date in all_months:
            ...
            gross_loss = group[group['PnL'] < 0]['PnL'].sum()
            ...
            pct_return = (net_profit / current_bal) * 100
        ...
        print("\n--- 2024-2025 MONTHLY BREAKDOWN (V3.2 STRICT FORTUNE) ---")
        ...
        print(f"\nGlobal Max Drawdown: {Global_DD:.2f}%")
        
        # Save Report
        with open("backtest_report_v3_2.md", "w") as f:
            f.write("# PropBot V3.2 (Strict Fortune) Report\n\n")
            f.write("## Config\n")
            f.write("- **Strategy**: V3.2 (Base Risk 0.75%, Aggressive Scaler > 0.5% DD)\n")
