...
class Backtester:
    ...
    def run_sim(self):
        ...
        for i in range(55, len(self.data)):
            ...
            if position != 0:
                ...
                if position == 1:
                    if curr_low <= sl_price: exit_price = sl_price; triggered = True
                    elif curr_high >= tp_price: exit_price = tp_price; triggered = True
                else: 
                    if curr_high >= sl_price: exit_price = sl_price; triggered = True
                    elif curr_low <= tp_price: exit_price = tp_price; triggered = True
            ...
            if position == 0 and risk_modifier > 0:
                ...
                is_extreme_rsi_buy = (rsi < 25); is_extreme_rsi_sell = (rsi > 75)
                ...
                if atr >= min_atr and adx_ok:
                    if close_prev > high_prev_2 and close_prev > open_prev and can_buy:
                        entry_price = current_open + spread; position = 1; signal_found = True
    ...
    def generate_report(self):
        ...
        for date in all_months:
            try: group = monthly_groups.get_group(date)
            ...
            avg_lot = group['Lots'].mean()
            
            # Profit Factor
            gross_win = group[group['PnL'] > 0]['PnL'].sum()
            gross_loss = abs(group[group['PnL'] < 0]['PnL'].sum())
            prof_factor = (gross_win / gross_loss) if gross_loss > 0 else gross_win
            ...
            if percent_ret >= 10: classification = "Excellent"
            elif percent_ret >= 5: classification = "Good"
            elif percent_ret < 0: classification = "Drawdown"
            if trades < 15: classification = "Dry/Slow"
            
            status = "Profitable" if net_profit > 0 else "Loss"
            
            report_data.append({
                "Month Name": date.strftime("%B %Y"),
                "Profit ($)": round(net_profit, 2),
                "Return %": round(percent_ret, 2),
                "Max DD %": round(max_dd, 2),
                "No of Trades": trades,
                "Win Rate %": round(win_rate, 1),
                "Avg Lot Size": round(avg_lot, 2),
                "Profit Factor": round(prof_factor, 2),
                "Status": status,
                "Month Classification": classification
            })
            
        df_report = pd.DataFrame(report_data)
        csv_filename = "Final_Bottest.csv" # Keep CSV for reference
        ...
        md_content += f"- **Period**: {Config.START_DATE} to {Config.END_DATE}\n"
        md_content += f"- **Account**: ${Config.INITIAL_BALANCE:,.2f}\n"
        
        total_profit = df_report['Profit ($)'].sum()
        final_bal = Config.INITIAL_BALANCE + total_profit
        total_ret = (total_profit / Config.INITIAL_BALANCE) * 100
        avg_monthly = df_report['Return %'].mean()
        max_dd = df_report['Max DD %'].min()
        win_rate = df_report['Win Rate %'].mean()
        
        md_content += "\n## Performance Summary\n"
        md_content += f"- **Total Net Profit**: **${total_profit:,.2f}**\n"
        md_content += f"- **Total Return**: **{total_ret:.2f}%**\n"
        md_content += f"- **Ending Balance**: **${final_bal:,.2f}**\n"
        md_content += f"- **Avg Monthly Return**: **{avg_monthly:.2f}%**\n"
        md_content += f"- **Max Drawdown**: **{max_dd:.2f}%**\n"
        md_content += f"- **Avg Win Rate**: **{win_rate:.1f}%**\n"
