...
# --- Configuration (V4 Final) ---
class Config:
    SYMBOLS = ["GC=F"] 
    START_DATE = "2021-01-01"
    END_DATE = "2026-02-01" # Up to Jan 31
    INTERVAL = "1h"    
...
class Backtester:
    ...
    def fetch_data(self):
        ...
        print(f"Actual End:   {self.data.index[-1]}")
        ...
        for i in range(55, len(self.data)):
            ...
            if position == 0 and risk_modifier > 0:
                ...
                if not (is_extreme_rsi_buy or is_extreme_rsi_sell):
                    if adx < 20: adx_ok = False # ADX Filter V4
    ...
    def generate_report(self):
        ...
        # Save Full CSV
        df_trades.to_csv("trade_log_full.csv", index=False)
        print("Detailed Trade Log saved to trade_log_full.csv")
        ...
        for date in all_months:
            ...
            # Max DD
            # Only look at this specific month window in equity curve
            ...
            current_bal += net_profit # Approximate rolling balance for return calc
        ...
        # Markdown Output
        md_content = "# Detailed Backtest Report (V4 Full History)\n\n"
        md_content += f"Period: {self.data.index[0].date()} to {self.data.index[-1].date()}\n"
        md_content += f"Start Balance: ${Config.INITIAL_BALANCE}\n"
        md_content += f"End Balance: ${self.balance:.2f}\n"
        md_content += "\n## Monthly Breakdown\n\n"
        md_content += df_report.to_markdown(index=False)
        
        with open("backtest_report_full.md", "w") as f:
            f.write(md_content)
        
        print("\n Report Generated in backtest_report_full.md")
